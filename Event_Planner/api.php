...
function loginUser($pdo) {
    ...
    try {
        // Query the User table using LEFT JOINs on Admin and SuperAdmin to compute the user's role.
        $sql = "SELECT 
                    U.UID,
                    U.password,
                    CASE
                        WHEN SA.UserID IS NOT NULL THEN 'superadmin'
                        WHEN A.UserID IS NOT NULL THEN 'admin'
                        ELSE 'student'
                    END AS role
                FROM `User` U
                LEFT JOIN Admin A ON U.UID = A.UserID
                LEFT JOIN SuperAdmin SA ON U.UID = SA.UserID
                WHERE U.email = :email
                LIMIT 1";
        $stmt = $pdo->prepare($sql);
        ...
        if ($user && $password === $user['password']) {
            // Store session data if desired.
            $_SESSION['user'] = [
                'uid'   => $user['UID'],
                'email' => $email,
                'role'  => $user['role']
            ];
            // Return both the UID and the computed role.
            echo json_encode(["success" => true, "uid" => $user['UID'], "role" => $user['role']]);
        } else {
...
function loadEventComments($pdo) {
    ...
    try {
        // Retrieve all comments for the given event, joined with the User table for first and last name.
        $sql = "SELECT C.UID, U.firstname, U.lastname, C.text, C.rating, C.timestamp 
                FROM Comment C
